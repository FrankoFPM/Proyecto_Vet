...
public class MetodosReadDelete {
    ...
    public static List<String[]> buscarRegistros(String tabla, String columna, String codigo) {
        ...
        try {
            ...
            if (resultado) {
                ...
                rs.close();
                if (!filas.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Registro encontrado", "Success",
    ...
    public static void Autenticar(Login lg, Dashboard_UI dash) {
        ...
        try {
            String query = "{call ValidarCredenciales(?,?,?,?)}"; // Añade un parámetro de salida para el rol
            ...
            cs_Login.registerOutParameter(4, Types.VARCHAR); // Registra el parámetro de salida para el rol
            cs_Login.execute();
            boolean resultado = cs_Login.getBoolean(3);
            String role = cs_Login.getString(4); // Obtiene el rol del usuario
            cs_Login.close();
            if (resultado) {
                JOptionPane.showMessageDialog(lg, "Inicio de sesión exitoso", "Success",
                        JOptionPane.INFORMATION_MESSAGE);
                lg.dispose();
                if (role != null) {
                    DashboardController controllerDash = new DashboardController(dash, user.toUpperCase(), role);
                } else {
                    JOptionPane.showMessageDialog(lg, "Error: el usuario no tiene un rol asignado", "Error",
